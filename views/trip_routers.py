...
def create_trip( req:TripBase, db: Session= Depends(get_db), current_user: UserBase  = Depends(get_current_user)):
    return trips.create_trip(db, req)

#get all trips
@router.get('/', response_model=List[TripDisplay])
def get_all_trips(user_id: int = None, db: Session=Depends(get_db), current_user: UserBase  = Depends(get_current_user)):
    return trips.get_all_trips(db, user_id)

#get trip
@router.get('/{id}', response_model=TripDisplay)
def get_trip(id: int, db: Session=Depends(get_db), current_user: UserBase  = Depends(get_current_user)):
    return trips.get_trip(db, id)

#update a trip
@router.put('/{id}',  response_model=TripDisplay)
def update_trip( req:TripBase, id: int, db: Session= Depends(get_db), current_user: UserBase  = Depends(get_current_user)):
...
def searsh_trip(departure_location: str, destination_location: str, departure_time: datetime,
                 available_adult_seats: int, available_children_seats: int  , db: Session=Depends(get_db), current_user: UserBase  = Depends(get_current_user)):
...
def delete_trip(id: int, user_id: int, db: Session=Depends(get_db), current_user: UserBase = Depends(get_current_user)):
