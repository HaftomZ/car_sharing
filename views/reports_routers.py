...
def create_report(request: ReportBase , db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
    return reports.create_report(db, request, current_user)


#get all reports for specific creator or reported user or all
@router.get('/', response_model=List[ReportDisplay])
def get_all_reports(creator_id: int = None, reported_id: int = None, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
...
def filter_reports(report_status: ReportStatus, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
    return reports.fiter_reports(db, report_status,current_user)


#get report
@router.get('/{id}', response_model=ReportDisplay)
def get_report(id: int, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
    return reports.get_report(db, id, current_user)


#handle report by admin
@router.put('/{id}')
def handle_report(id: int, report_status: ReportStatus, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
