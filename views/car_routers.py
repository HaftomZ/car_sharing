...
def get_all_cars(user_id: int = None, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
...
def update_car(request: CarBase, id: int, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
    return cars.update_car(db, id, request, current_user)

#delete car that is related to a user
@router.delete('/{id}' , status_code=status.HTTP_204_NO_CONTENT)
def delete_car(id: int, db: Session=Depends(get_db), current_user: userDisplay = Depends(get_current_user)):
