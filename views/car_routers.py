...
def get_all_cars(user_id: int = None, db: Session=Depends(get_db), current_user: UserBase  = Depends(get_current_user)):
    return cars.get_all_cars(db, user_id)


#get car
@router.get('/{id}', response_model=CarDisplay)
def get_car(id: int, db: Session=Depends(get_db), current_user: UserBase = Depends(get_current_user)):
    return cars.get_car(db, id)


#update car that is related to a user
@router.put('/{id}', response_model=CarDisplay)
def update_car(request: CarBase, id: int, db: Session=Depends(get_db), current_user: UserBase = Depends(get_current_user)):
    return cars.update_car(db, id, request)

#delete car that is related to a user
@router.delete('/{id}' , status_code=status.HTTP_204_NO_CONTENT)
def delete_car(id: int, user_id: int, db: Session=Depends(get_db), current_user: UserBase = Depends(get_current_user)):
