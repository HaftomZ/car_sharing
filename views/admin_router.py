...
def get_all_admins(db: Session = Depends(get_db), current_user: UserBase | AdminBase = Depends(get_current_user)):
    return admins.get_all_admins(db)

@router.get('/{id}', response_model=AdminDisplay)
def get_admin(email: str, password: str, db: Session = Depends(get_db), current_user: UserBase | AdminBase = Depends(get_current_user)):
    return admins.login_admin(db, email, password)

@router.put('/{id}')
def update_admin(admin_role: AdminRole, id: int, request: AdminBase, db: Session = Depends(get_db),current_user: UserBase | AdminBase = Depends(get_current_user)):
    return admins.update_admin(db, id, request, admin_role.value )

@router.delete("/{id}")
def delete_admin(id: int, db: Session = Depends(get_db),current_user: UserBase | AdminBase = Depends(get_current_user)):
